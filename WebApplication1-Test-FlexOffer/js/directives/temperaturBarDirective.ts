...
module app.Directives {
    ...
    export function ngTem($window: Window): ng.IDirective {
        ...
        return {
            ...
            link(scope, element: JQuery, attrs: ng.IAttributes) {
                ...
                scope.render = function (temperaturesData: Array<Temperature>) {
                    ...
                    var date = [];
                    date = temperaturesData.map(function (d) { return parseDate(d.date); });

                    // Scales
                    var xScale = d3.time.scale()
                        .domain(d3.extent(date))
                        .range([barWidth, width - barWidth / 2]);
                    ...
                    svg.select('g')
                        .append("g")
                        .attr("class", "axis")
                        .attr('transform', 'translate(' + yAxisPaddaing + ',0)')
                        .call(yAxis)
                        .append("text")
