            ...
            return {
                ...
                link: function (scope, element, attrs) {
                    ...
                    scope.render = function (timesliceData, scheduleData) {
                        ...
                        // Scales
                        var xScale = d3.time.scale().domain(d3.extent(date)).range([timeTicks, width - timeTicks]);
                        ...
                        var color = d3.scale.ordinal().range([minimum, flex]).domain(d3.keys(timesliceData[0]).filter(function (key) {
                        ...
                        var consumption = svg.select('g').selectAll(".consumption").data(timesliceData).enter().append("g").attr("class", "g").attr("transform", function (d) {
                            return "translate(" + (xScale(parseDate(d.date)) - timeTicks) + ",0)";
                        });
